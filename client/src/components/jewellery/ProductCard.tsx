...
import { formatFullPrice, fromPriceInr } from "@shared/jewellery/pricing";
    ...
    <Link href={pieceHref(piece)} className="jw-card">
      ...
      <div className="jw-card-body">
        ...
        <div className="jw-card-foot">
          ...
          {hidePrice ? <span className="jw-card-price">{piece.code}</span> : <span className="jw-card-price">{formatFromPrice(piece, currency)}{from != null && showLaunchOffer() ? <s aria-label="Full price">{formatFullPrice(from, currency)}</s> : null}</span>}
        </div>
      </div>
    </Link>
  );
