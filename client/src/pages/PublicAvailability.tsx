...
function SelectField({ label, values, value, onChange, all, disabled = false }: { label: string; values: string[]; value: string; onChange: (value: string) => void; all: string; disabled?: boolean }) {
  return <label className="catalog-filter"><span>{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} disabled={disabled}><option value="">{all}</option>{values.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>;
}
...
export default function PublicAvailability({ locale = "global" }: { locale?: Locale }) {
  ...
  const queryOptions = { staleTime: 30_000, retry: 1, retryDelay: 1_000, refetchOnWindowFocus: false } as const;
  const catalog = trpc.availability.profiles.useQuery(filterInput, queryOptions);
  const coreSummary = trpc.availability.summary.useQuery(undefined, queryOptions);
  const statementSummary = trpc.availability.summary.useQuery(statementSummaryInput, { ...queryOptions, enabled: isStatement });
  const activeSummary = trpc.availability.summary.useQuery(activeSummaryInput, queryOptions);
  ...
  const summaryUnavailable = coreSummary.isError || statementSummary.isError || activeSummary.isError;
  const retryAvailability = () => {
    void catalog.refetch();
    void coreSummary.refetch();
    if (isStatement) void statementSummary.refetch();
    void activeSummary.refetch();
  };

  useEffect(() => {
    if (!catalog.isLoading || typeof window === "undefined") {
      setLoadingLonger(false);
      return;
    }
    const timeout = window.setTimeout(() => setLoadingLonger(true), 8_000);
    return () => window.clearTimeout(timeout);
  }, [catalog.isLoading]);

  const catalogRequestStartedAt = useRef<number | null>(null);
  useEffect(() => {
    if (catalog.isFetching && catalogRequestStartedAt.current === null) {
      catalogRequestStartedAt.current = Date.now();
      return;
    }
    if (catalog.isFetching || catalogRequestStartedAt.current === null || typeof window === "undefined") return;
    const tracker = (window as Window & { umami?: { track?: (event: string, data: Record<string, unknown>) => void } }).umami;
    const latencyMs = Date.now() - catalogRequestStartedAt.current;
    tracker?.track?.(catalog.isError ? "availability_catalog_error" : "availability_catalog_loaded", {
      tab,
      locale,
      latency_ms: latencyMs,
      result_count: catalog.data?.profiles.length ?? 0,
    });
    catalogRequestStartedAt.current = null;
  }, [catalog.data, catalog.isError, catalog.isFetching, locale, tab]);
  ...
  return <div className="catalog-shell"><PublicMetadata locale={locale} page="availability" /><header className="catalog-header"><Link href={menuLinks[locale]} className="brand"><span className="brand-name">ALVORA</span></Link><nav><a href={menuLinks[locale]}>{text.made}</a><a href="#catalog-filter">{text.filter}</a><a href="#catalog-collections">{text.fancy} / {text.white} / {text.statement}</a></nav><Link href={menuLinks[locale]} className="catalog-back">← Alvora</Link></header>
    <main>
      ...
      <section className="catalog-controls" id="catalog-filter"><div className="catalog-filter-title"><SlidersHorizontal size={16} /><span>{text.filter}</span></div>{summaryUnavailable && <p className="catalog-filter-note" role="status">{text.filterUnavailable}</p>}<div className="catalog-filter-fields"><SelectField label={text.shape} values={filters.shapes} value={shape} onChange={resetPage(setShape)} all={text.all} disabled={summaryUnavailable} /><SelectField label={text.carat} values={filters.caratBands} value={caratBand} onChange={resetPage(setCaratBand)} all={text.all} disabled={summaryUnavailable} /><SelectField label={text.colour} values={filters.colours} value={colour} onChange={resetPage(setColour)} all={text.all} disabled={summaryUnavailable} /><SelectField label={text.clarity} values={filters.clarities} value={clarity} onChange={resetPage(setClarity)} all={text.all} disabled={summaryUnavailable} />{isStatement && <><SelectField label={text.type} values={filters.types} value={statementType} onChange={resetPage(setStatementType)} all={text.all} disabled={summaryUnavailable} /><SelectField label={text.lab} values={filters.labs} value={lab} onChange={resetPage(setLab)} all={text.all} disabled={summaryUnavailable} /></>}</div><label className="catalog-sort"><span>{text.sort}</span><select value={sort} onChange={(event) => { setSort(event.target.value as typeof sort); setPage(0); }}><option value="curated">{text.curated}</option><option value="carat_desc">{text.caratDesc}</option><option value="carat_asc">{text.caratAsc}</option><option value="new_arrivals">{text.newArrivals}</option></select></label><div className="catalog-view-toggle"><button className={view === "grid" ? "is-active" : ""} onClick={() => setView("grid")} aria-label="Grid view"><Grid2X2 size={16} /></button><button className={view === "list" ? "is-active" : ""} onClick={() => setView("list")} aria-label="List view"><List size={16} /></button></div><button className="catalog-download" type="button" disabled={!catalog.data?.profiles.length || downloadCurrentView.isPending} onClick={() => downloadCurrentView.mutate({ collection: isStatement ? "statement" : "core", stoneIds: catalog.data?.profiles.map((stone) => stone.id) ?? [] })}><Download size={14} /> {downloadCurrentView.isPending ? text.preparing : text.download}</button>{downloadCurrentView.error && <p className="catalog-download-error" role="alert">{downloadCurrentView.error.message}</p>}</section>
      <div id="catalog-collections" className="catalog-results" aria-live="polite" aria-busy={catalog.isLoading || catalog.isFetching}>{catalog.isLoading ? <section className={view === "grid" ? "catalog-grid" : "catalog-list"}><p className="catalog-loading"><Loader2 className="animate-spin" /> Loading current production…</p>{loadingLonger && <p className="catalog-loading-note">{text.takingLonger} <button type="button" onClick={retryAvailability}>{text.retry}</button></p>}</section> : catalog.isError ? <section className="catalog-state catalog-error-state" role="alert"><p>{text.loadError}</p><button type="button" onClick={retryAvailability}>{text.retry}</button></section> : visibleStones.length ? <>{pinnedStones.length > 0 && <section className="catalog-curated"><header><p className="eyebrow"><span /> {text.picks}</p></header><div className={view === "grid" ? "catalog-grid catalog-curated-grid" : "catalog-list catalog-curated-list"}>{pinnedStones.map((stone) => <CatalogStoneCard key={stone.id} stone={stone} isStatement={isStatement} tab={tab} locale={locale} view={view} onOpenViewer={(url, stockNumber) => setViewer({ url, stockNumber })} />)}</div></section>}{regularStones.length > 0 && <section className={view === "grid" ? "catalog-grid" : "catalog-list"}>{regularStones.map((stone) => <CatalogStoneCard key={stone.id} stone={stone} isStatement={isStatement} tab={tab} locale={locale} view={view} onOpenViewer={(url, stockNumber) => setViewer({ url, stockNumber })} />)}</section>}</> : <section className={view === "grid" ? "catalog-grid" : "catalog-list"}><p className="catalog-empty">{text.empty}</p></section>}</div>
