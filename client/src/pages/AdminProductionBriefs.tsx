...
export default function AdminProductionBriefs() {
  ...
  return <DashboardLayout><div className="admin-shell">
    <header className="admin-topline">
      ...
      <div className="admin-brief-header-actions"><button className="admin-export-button" type="button" onClick={downloadCsv} disabled={exportCsv.isFetching}><Download size={15} /> {exportCsv.isFetching ? "Preparing…" : `Export ${marketFilter === "all" ? "CSV" : `${marketFilter} CSV`}`}</button><ClipboardList size={25} className="text-[#c9ff63]" /></div>
    </header>
    ...
    {briefs.isLoading ? <Loader2 className="animate-spin text-[#c9ff63]" /> : <div className="admin-brief-list">
      {visibleBriefs.map((item) => <article className="admin-brief" key={item.id}>
        ...
        <p className="admin-bands">{item.requestType} · {item.yearsTrading} years trading · references {item.tradeReferencesAvailable}</p>
