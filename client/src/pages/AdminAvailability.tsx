...
import { ArchiveRestore, Database, FileCheck2, Loader2, Upload } from "lucide-react";
...
type ImportPreview = {
  ...
  whiteRowCount: number;
  fancyRowCount: number;
...
export default function AdminAvailability() {
  ...
  return <DashboardLayout><div className="admin-shell availability-admin-shell">
    ...
    <section className="availability-admin-card"><header><div><p className="portal-kicker">IMPORT VALIDATION</p><h2>Catalog acceptance</h2></div><FileCheck2 size={19} /></header><p className="admin-empty">The active catalog accepts structurally valid White and Fancy Colour records. Any missing identifier, certificate, verification URL, invalid numeric detail, or malformed supplied video URL is blocked in the upload rejection report before a snapshot can be activated.</p></section>

    <section className="availability-admin-card"><header><div><p className="portal-kicker">RECOVERY</p><h2>Snapshot history</h2></div><ArchiveRestore size={19} /></header>{versions.data?.length ? <div className="availability-version-list">{versions.data.map((version) => <article key={version.id}><div><strong>{version.sourceFilename}</strong><span>{version.rowCount} rows · {version.fancyRowCount} Fancy Colour · {version.whiteRowCount} White · {new Date(version.activatedAt).toLocaleString()}</span></div>{version.status === "active" ? <em>Active</em> : <button onClick={() => restore.mutate({ importId: version.id })} disabled={restore.isPending}>Restore this version</button>}</article>)}</div> : <p className="admin-empty">No availability snapshots have been imported.</p>}</section>
  </div></DashboardLayout>;
}
