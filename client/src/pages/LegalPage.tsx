...
const legalCopy = {
  privacy: {
    ...
    sections: [
      ...
      ["What we collect", "When you send a production brief, we collect your name, work email, optional company or workshop, years trading, whether trade references are available, preferred first-order approach, the make you describe, and the market or country context of the form. We also retain the time of the enquiry, follow-up information created by authorised staff, and the result of the internal delivery attempt."],
      ["Why we use it", "We use this information to understand and respond to a requested make, assess whether an account conversation is appropriate, prepare a quote or production discussion, maintain an internal enquiry record, and protect the security and operation of the service. Depending on the context, this may involve taking steps you request before a trade relationship and our legitimate interest in operating a secure business-to-business enquiry process."],
      ["Who receives it", "Authorised Alvora staff can access production briefs. We use Resend to deliver internal enquiry alerts; it acts as an email-delivery service provider. We also use hosting, database, storage, and authentication providers to operate the site. We do not sell production-brief information or use it for advertising audiences."],
      ["Analytics and cookies", "The current public site uses Umami for privacy-first, cookie-free analytics. The configured tracker records standard anonymous measurement information such as page views, referrer URL, browser, device type, and country. No custom analytics event names are configured in the public route code. We do not load advertising pixels, session replay, or cross-site tracking on the public site, so there is no consent banner in the current release. If non-essential cookies or similar tracking are added, this page and the consent experience will be updated before that technology loads."],
  ...
  terms: {
    ...
    sections: [
      ...
      ["Quotes", "A quote is valid only until the expiry stated in that quote. If no expiry is stated, availability, price, delivery timing, and specification remain subject to Alvora’s written confirmation before an order is accepted."],
      ["Accounts and payment", "Payment terms are confirmed per account and per confirmed order. The public site does not publish credit-term numbers. A first order may be used to confirm the make and account process; ongoing trade terms depend on the agreed account arrangement, credit review, and references where applicable."],
      ["Orders and dispatch", "A production commitment and dispatch arrangement take effect only after Alvora confirms the relevant order, specification, commercial terms, and delivery details in writing. Dispatch is made on confirmed orders, using the agreed insured delivery method and shipment documentation."],
...
export default function LegalPage({ page }: { page: LegalPageKind }) {
  ...
  return <div className="legal-shell">
    <header className="legal-header"><a className="brand" href="/" aria-label="Alvora home"><span className="brand-name">ALVORA</span></a><a className="legal-back" href="/"><span>Back to Alvora</span><ArrowUpRight size={15} /></a></header>
    ...
    <footer className="site-footer legal-footer"><div className="footer-brand"><span className="brand-name">ALVORA</span></div><p>Lab-grown diamond manufacturing<br />Surat, India</p><nav className="footer-legal" aria-label="Legal information"><a href="/privacy">Privacy</a><a href="/terms">Terms of trade</a></nav><a href="#top">Top <ArrowUpRight size={15} /></a></footer>
  </div>;
}
